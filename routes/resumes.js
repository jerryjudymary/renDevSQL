...
router.get("/", async (req, res) => {
  try {
    ...
    await connection.query("SELECT * FROM resumes", (error, result, fields) => {
      if (error) throw error;
...
router.put("/:resumeId", authMiddleware, upload.single("resumeImage"), async (req, res) => {
  try {
    ...
    await connection.query(existResumid, (error, result, fields) => {
      if (error) throw error;
      ...
      if (userId !== existResume.userId) {
        ...
        role='${role}', skills='${skills}', content2='${content2}', content3='${content3},
        WHERE resumeId='${resumeId} AND userId = '${userId}'`;

        connection.query(Resumesput, (error, result, fields) => {
          if (error) throw error;
  ...
  try {
    ...
    await connection.query(existResumid, (error, result, fields) => {
      ...
      if (userId !== existResum.userId) {
        ...
        connection.query(existResumdel, (error, result, fields) => {
          if (error) throw error;
