...
const { User, Project, Application, ProjectSkill, Resume, ResumeSkill, ProjectPhoto, sequelize } = require("../../../models");
...
exports.userProject = async (req, res) => {
  try {
    ...
    if (userId === undefined) {
      ...
      const sql = `SELECT A.projectId, id, nickname, title, details, role, start, end, createdAt,
...
exports.userResume = async (req, res) => {
  try {
    ...
    if (userId === undefined) {
      ...
      const sql = `SELECT A.resumeId, id, nickname, content, content2, content3, role, start, end, createdAt,
...
exports.myApply = async (req, res) => {
  try {
    ...
    if (user === undefined) {
      ...
      if (proId) {
        const sql = `SELECT A.*, JSON_ARRAYAGG(JSON_OBJECT( 
...
exports.recruit = async (req, res) => {
  try {
    ...
    const recruit = query2
      .map((data) => data.resumeId)
      .filter((el) => {
        return el !== null;
      })
      .toString();
    ...
    if (recruit) {
      const sql3 = `SELECT A.*, JSON_OBJECT('nickname', resume.nickname, 'role', resume.role, 'resumeId', resume.resumeId ) as Resume
    FROM (
