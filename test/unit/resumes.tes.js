...
//   });
//   it("전체 조회 호출", async () => {
//     // Resume.findAll = jest.fn().mockReturnValue(Resumefind);
//     const query = jest.fn().mockReturnValue(Resumefind);
//     const resume = sequelize.query(query, { type: QueryTypes.SELECT });
//     await resumeController.resumeInfo(req, res, next);
//     // expect(query).toBeCalled();
//     // expect(res._getJSONData()).toStrictEqual({ Resumefind });
//     expect(res._getData()).toBe(Resumefind);
//     // expect(resume).toBe({ returnResumes: Resumefind });
//     // expect(res._getData()).toHaveReturned();
//   });
...
//   it("팀원 찾기 전체 조회 상태코드 400 반환", async () => {
//     await resumeController.resumeInfo(req, res, next);
