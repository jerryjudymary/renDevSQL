...
const Resumefind = require("../data/Allresume.json");
...
describe("팀원 찾기 등록 테스트", () => {
  ...
  it("팀원 찾기 등록 상태코드 200 반환", async () => {
    ...
    expect(res._getJSONData()).toEqual({ message: "나의 정보를 등록 했습니다." });
  });
...
describe("팀원 찾기 전체 조회 테스트", () => {
  it("전체 조회 호출", async () => {
    // const mreturnResumes = {
    //   const query = jest.fn().mockReturnValue(Resumefind),
    // };
    // const returnResumes = sequelize.query(query, { type: QueryTypes.SELECT });
    // const returnResumes = jest.fn().mockReturnValue(Resumefind);
    // await resumeController.resumeInfo(req, res, next);
    const query = jest.fn().mockReturnValue(Resumefind);
    // expect(query).toBeCalled();
    // expect(res._getJSONData()).toStrictEqual({ Resumefind });
    // expect(res._getJSONData()).toEqual(returnResumes);
    expect(query).toEqual(Resumefind);
    // expect(res._getData()).toHaveReturned(returnResumes);
  });

  it("팀원 찾기 전체 조회 상태코드 200 반환", async () => {
    await resumeController.resumeInfo(req, res, next);

    expect(res._getJSONData()).toEqual(Resumefind);
    // expect(res.statusCode).toBe(200);
